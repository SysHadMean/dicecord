...
function gestionDe100(value,user,ServerName) {
    ...
    if (parseInt(resultat,10) == Include.context1) {
        console.info("critique mixte ("+Include.context1+")");
        db.insert(user, "100", resultat, "mixte", ServerName);
        return resultat + "\n" + jet.mixteCritique();
    } else if (parseInt(resultat,10) == Include.context2) {
        console.info("critique mixte ("+Include.context2+")");
        db.insert(user, "100", resultat, "mixte", ServerName);
        return resultat + "\n" + jet.mixteCritique();
    } else if (parseInt(resultat,10) <= Include.borneReussite) {
        console.info("réussite critique");
        db.insert(user, "100", resultat, "reussite", ServerName);
        return resultat + "\n" + jet.reussiteCritique();
    } else if (parseInt(resultat,10) >= Include.borneEchec) {
...
function topTier(server,ServerName) {
    ...
    if (topTier.length === 5) {
        return "\nTu veux des stats ?" +
                "\n" + "Alors accroche toi bien, c'est partit pour le top 5 des joueurs les plus actifs du serveur " + server + " :" +
                ...
                "\n" + "4 - " + topTier[3].USER + " avec " + topTier[3].COUNT + " lancés !!" +
                "\n" + "5 - " + topTier[4].USER + " avec " + topTier[4].COUNT + " lancés !!" +
                "\n" + "Tu as intérêt a faire rouler tes dés coco si tu veux être dans mon classement !!";
        } else {
            console.info("Alerte pas assez de joueur");
            return "Il n'y a pas eu suffisament de lancés pour sortir un top 5";
        }
}
...
const message = {
    gestionDe(contenu,user,server,ServerName) {
        ...
        if (regDeComplexe.test(contenu) === true) {
            ...
            if (check === "OK") {
                if (parseInt(argDe[0],10) >= 1 && (parseInt(argDe[1],10) === 100)) {
    ...
    gestionText(messageText,user,server,ServerName) {
        ...
        switch (messageText) {
            case "help":
                retour="Usage : "+Include.prefix+"+xdy+z (ex: "+Include.prefix+"1d100 ou "+Include.prefix+"4d6 ou "+Include.prefix+"1d10+3) \nLes dés sont entre 1 et leur valeur";
                break;    
            case "stat":
