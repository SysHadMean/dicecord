...
const bdd = {
    ...
    createTable(ServerName) {
        ...
        let stmt = db.prepare("CREATE TABLE IF NOT EXISTS jet_" + ServerName + " (USER TEXT,VALEUR_DE TEXT,RESULTAT NUMERIC,CRITIQUE TEXT, DATE DATETIME)");
        stmt.run();
        db.close();
    },
    ...
    statAll(ServerName) {
        ...
        creaTable.run();
        let selectLim = db.prepare("SELECT * FROM stat_" + ServerName + " LIMIT 5");
